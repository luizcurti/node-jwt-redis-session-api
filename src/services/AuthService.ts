...
export class AuthService {
  ...
  async login(rawInput: unknown): Promise<LoginResult> {
    ...
    // The session write above is load-bearing for auth and must fail the
    // login if Redis is unreachable. This cache write is a performance
    // optimization for GET /users/profile/:id, not part of the auth
    // contract — a failure here must not fail a login with valid
    // credentials. Worst case, the next profile fetch 404s until the cache
    // is repopulated.
    try {
