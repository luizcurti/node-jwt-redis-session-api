...
function buildApp(overrides: Parameters<typeof createLoginRateLimiter>[1]) {
  const app: Express = express();
  app.post(
    '/login',
    createLoginRateLimiter(testRedisClient, {
      skip: () => false,
      ...overrides,
    }),
    (_req, res) => {
...
describe('createLoginRateLimiter (integration, real Redis)', () => {
  beforeEach(async () => {
    await resetCache();
  });
  ...
  it('is skipped by default when NODE_ENV is test', async () => {
    ...
    app.post(
      '/login',
      createLoginRateLimiter(testRedisClient, { windowMs: 1000, max: 1 }),
      (_req, res) => {
        res.status(200).json({ ok: true });
      }
    );
