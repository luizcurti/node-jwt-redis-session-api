...
/**
 * A destination can be injected (e.g. an in-memory stream in tests) instead
 * of the default stdout — pino writes to its destination directly (via
 * sonic-boom), bypassing `process.stdout.write`, so that's the only way to
